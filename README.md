...
- **📊 Comprehensive Data** - 28 data sections with 70+ Mac models in SMBIOS database
...
- **🔍 OCLP-Aware** - 6 detection methods for OpenCore Legacy Patcher
- **🎨 Apple HIG TUI** - Professional terminal UI with Apple Human Interface Guidelines design
- **🌐 Multi-Format** - JSON, TXT (AI-optimized), HTML dashboard, Interactive TUI
...
| **Test Coverage** | 113 tests | 100% pass rate, 51% coverage |
| **SMBIOS Database** | 70 models | Intel + Apple Silicon |
...
3. **HTML** (~180KB) - Dark-themed Bootstrap 5 dashboard (requires internet for CDN)
...
- **System**: macOS version, SMBIOS enrichment (70 models), SIP/FileVault/Gatekeeper status
...
- SMBIOS mismatch detection (unsupported OS)
...
macos-prose --html
# Note: HTML dashboard requires internet connection for Bootstrap CSS and icons (CDN)
...
│   ├── html_report.py            # Bootstrap 5 dashboard generator (1 function)
...
│   │   └── smbios.py             # Mac model database (70 models, 5 functions)
...
├── tests/                        # Test suite (2,412 lines, 113 tests)
│   ├── conftest.py               # Pytest fixtures (5 profiles)
│   ├── test_smbios.py            # SMBIOS tests (18 tests)
...
│   ├── test_html_report.py       # HTML tests (3 tests)
...
│   └── smbios_models.json        # 70 Mac models with metadata
├── scripts/                      # Maintenance scripts
│   ├── scrape_macos_versions.py  # Update macOS version data
│   └── scrape_smbios_models.py   # Update SMBIOS model database
...
70 Mac models from 2006-2024, including:

- **MacBook**: Core Duo to Apple Silicon M3
- **MacBook Air**: 2008-2024 (Intel + M1/M2/M3)
- **MacBook Pro**: 2006-2024 (Intel + M1/M2/M3 Pro/Max)
- **iMac**: 2006-2024 (Intel + M1/M3)
- **Mac mini**: 2007-2024 (Intel + M1/M2)
- **Mac Pro**: 2006-2023 (Intel + M2 Ultra)
- **Mac Studio**: 2022-2024 (M1/M2 Max/Ultra)

Each model includes:

- Marketing name (e.g., "MacBook Air (13-inch, Mid 2013)")
- Board ID (e.g., "Mac-7DF21CB3ED6977E5")
- CPU generation (Intel or Apple Silicon)
- Maximum supported macOS version
- Stock GPU configurations
...
- ✅ **Completed**: JSON/TXT/HTML reports, Interactive TUI, Diff mode, Full type safety, 113 tests, Python 3.9-3.14
- 🚧 **In Progress**: PyPI package publication
- 📋 **Planned**: Homebrew formula, Plugin architecture, Historical tracking, PDF/Markdown export
